...
; sign toggles below. The engine renders Y-flipped (ogl_setPersp writes
...
; both renderers. The engine owns movement collision and triggers; fixed and
; cinematic cameras keep their framing, because
; those shots are placed deliberately by the level.
;
; Turning uses a stable VR heading; looking and leaning come from the
...
; Translation is relative to the head position captured on entry. END retakes
; it without changing your world heading. Rotation is always tracked.
...
; The setting exists because the code path is cheap and correct, not because
; this game needs it:
;
;   * TR4-6 had a vid_setPerspOffset function whose whole job was writing these
;     two terms, and it laid the inventory out with them. TR1-3 has NO SUCH
;     FUNCTION -- there is no engine-side shear lever anywhere in tomb123.exe.
...
;
; What the code does IF it ever fires, recorded because both halves were got
; wrong the first time on TR4-6 and both were visible on a headset:
;
;   * The offset is re-applied as a SKEW of the ENGINE's camera space, not as a
;     shift of the headset's NDC, and it goes innermost -- AFTER the per-eye
;     transform. A projection carrying shear s equals the same projection
;     without it applied to space pre-skewed by (s/scale)*z, so the skew
;     reproduces the placement whatever else has been done to the matrix.
;     Adding the shear onto the eye frustum instead puts it on the far side of
;     the head transform, and the offset then swings with head orientation.
;   * It is converted ANGLE to ANGLE by dividing through the engine's own
;     m00/m11, not copied as a raw NDC value. The frustums differ: measured on
;     TR4-6, engine tanX 1.119 vs eye 1.108, but engine tanY 0.629 vs eye 1.197.
;     Copying the NDC threw the vertical placement 1.9x too far, out into the
;     lens distortion. That was the fishbowl.
;
; 0 = do not re-apply it, for A/B.
PreserveProjOffset=1

; Trim on the whole offset, and inert for the same reason as the setting above.
; 1.0 reproduces the engine's layout exactly; lower values pull a shear-placed
; row in toward the centre; 0 collapses it into one stack.
ProjOffsetScale=1.0
...
;
; Unlike PreserveProjOffset above, this one IS expected to fire here:
; vid_setOrtho3D is a real 78-byte function in tomb123.exe (RVA 0x0000A590) and
; the game calls it. On TR4-6 the equivalent path never fired once across a TR4
; and a TR5 session. The ortho3D= counter in the health report says what it is
; actually doing on the screen in front of you.
;
