...
void VideoPanel::Replay(const mat4& mvp, bool flipV) {
    ...
    // been behind its back, so invalidate the parts we disturbed.
    //
    // TR1-3 differs from TR4-6 here: RenderState has a single `mesh` pointer
    // rather than separate vb/ib, and shaders[] has 74 entries rather than 202,
    // so the out-of-range sentinel is kShaderCount and not 0xCA. Writing 0xCA
    // would still be out of range and would still work, but it would be a number
    // with no meaning on this build.
    //
    // validate_draw compares vid_state_prev.shader against vid_state.shader and
    // reloads the program on any difference, so any value the engine cannot
    // legitimately hold forces the reload. Nulling `mesh` forces the vertex
    // array and buffers to be rebound on the next draw, and 0xFFFFFFFF in tex[0]
    // forces the texture binding.
    VidStatePrev().shader = kShaderCount;
    VidStatePrev().mesh   = nullptr;
